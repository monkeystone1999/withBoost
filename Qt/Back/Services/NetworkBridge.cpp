...
NetworkBridge::NetworkBridge(INetworkService *service, QObject *parent)
  ...
  connect(debounceTimer_, &QTimer::timeout, this,
          &NetworkBridge::processPendingStates);
}
...
void NetworkBridge::connectToServer(const QString &host, const QString &port,
  ...
  cbs.onLoginFail = [this, purpose](const std::string &s) {
    QMetaObject::invokeMethod(
        this,
        [this, purpose, q = QString::fromStdString(s)] {
          const auto doc = QJsonDocument::fromJson(q.toUtf8());
          ...
          if (msg.isEmpty()) {
            msg = q; // Fallback to raw string if JSON doesn't contain
                     // error/message
          }
  ...
  cbs.onMetaResult = [this](const std::string &s) {
    QMetaObject::invokeMethod(
        this,
        [this, q = QString::fromStdString(s)] { emit metaResultReceived(q); },
        Qt::QueuedConnection);
  };
...
void NetworkBridge::sendDevice(const QString &deviceIp, const QString &motor,
  ...
  // 1. Determine if this is an "Action" (Movement or manual ON/OFF)
  //    Actions are sent immediately.
  bool isAction = false;
  if (!motor.isEmpty() && motor != "auto" && motor != "unauto")
    isAction = true;
  if (ir == "on" || ir == "off")
    isAction = true;
  ...
  if (isAction) {
    ...
    service_->send(static_cast<uint8_t>(0x04), body);
  }

  // 2. Always update the pending state for status synchronization.
  //    If motor was a movement action, follow up with "unauto".
  DeviceState &st = pendingStates_[deviceIp];
  bool updated = false;
  if (!motor.isEmpty()) {
    if (motor != "auto" && motor != "unauto") {
      // st.motor = "unauto"; // Hardcoded follow-up
    } else {
      st.motor = motor;
      updated = true;
    }
  ...
  // Only restart the timer if we actually have a status update to sync.
  if (updated) {
    debounceTimer_->start();
  }
}

void NetworkBridge::processPendingStates() {
  for (auto it = pendingStates_.begin(); it != pendingStates_.end(); ++it) {
    const QString &ip = it.key();
    const DeviceState &st = it.value();

    if (st.motor.isEmpty() && st.ir.isEmpty() && st.heater.isEmpty())
      continue;

    const auto body = buildJson({{"device", ip.toStdString()},
                                 {"motor", st.motor.toStdString()},
                                 {"ir", st.ir.toStdString()},
                                 {"heater", st.heater.toStdString()}});
    service_->send(static_cast<uint8_t>(0x04), body);
  }
