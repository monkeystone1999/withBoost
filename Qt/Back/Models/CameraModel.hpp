...
struct CameraEntry {
  ...
  DeviceInfo deviceInfo;          // Latest sensor batch
};
...
public:
  ...
  Q_INVOKABLE void swapCameras(int indexA, int indexB) {
...
public slots:
  ...
  // Called by Core when META telemetry is received
  void updateSensorInfo(const QString &cameraId, double hum, double light,
                        double tilt, double tmp);

signals:
  // Emits the new SlotInfo list for VideoManager::registerSlots
  void slotsUpdated(QList<SlotInfo> slots);
  // Replaced URL-only signal with CameraID-only signal
  ...
  // Fired when new sensor telemetry is available
