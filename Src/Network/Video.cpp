...
int Video::interrupt_cb(void *ctx) {
  auto *v = static_cast<Video *>(ctx);
  return v->isStopping() ? 1 : 0;
}

void Video::startStream(const std::string &url, int fpsLimit) {
  if (url.empty())
    return;
  stopStream();
  stopThread_ = false;
...
bool Video::tryOnceFFmpeg(const std::string &url) {
  ...
  int openRet =
      avformat_open_input(&formatCtx_, url.c_str(), nullptr, &options);
  if (openRet != 0) {
  ...
  while (!stopThread_) {
    int readRet = av_read_frame(formatCtx_, packet);
    if (readRet < 0)
      break;
    ...
    if (packet->stream_index == videoStreamIndex_) {
      ...
      int receiveRet = 0;
      while ((receiveRet = avcodec_receive_frame(codecCtx_, frame)) == 0) {
        AVFrame *targetFrame = frame;

        if (frame->format == AV_PIX_FMT_D3D11) {
          int hwRet = av_hwframe_transfer_data(swFrame, frame, 0);
          if (hwRet < 0) {
        ...
        // --- FPS throttle (dynamic — reads atomic fpsLimitMs_) ---
        ...
        if (elapsedMs < limitMs) {
          av_frame_unref(frame);
          // Drain remaining decoded frames to avoid decoder stall
          while (avcodec_receive_frame(codecCtx_, frame) == 0)
            av_frame_unref(frame);
          // Yield CPU for the remaining interval
          auto sleepMs = limitMs - elapsedMs;
          if (sleepMs > 0)
            std::this_thread::sleep_for(std::chrono::milliseconds(sleepMs));
          break; // back to av_read_frame loop
        }
        lastFrameTime_ = now;
