        ...
        if (elapsedMs < limitMs) {
          ...
          if (limitMs - elapsedMs > 0)
            std::this_thread::sleep_for(
                std::chrono::milliseconds(limitMs - elapsedMs));
          break;
        }
        lastFrameTime_ = now;
 ...
 *
 * **[VideoEngine 구현 상세 가이드]**
 *
 * 1. FFmpeg 옵션 최적화:
 *    - `rtsp_transport`: `tcp`를 사용하여 패킷 손실에 따른 화질 저하를
 * 방지합니다.
 *    - `fflags`: `nobuffer`를 설정하여 실시간성을 극대화합니다.
 *    - `stimeout`: 네트워크 타임아웃을 3초로 설정하여 빠른 재연결을 도모합니다.
 *
 * 2. 하드웨어 가속 절차:
 *    - `av_hwdevice_ctx_create`를 통해 D3D11VA 장치를 초기화합니다.
 *    - 디코딩된 프레임이 GPU 공간에 있을 경우(AV_PIX_FMT_D3D11),
 *      `av_hwframe_transfer_data`를 호출하여 시스템 메모리(NV12)로
 * 변환/복사합니다.
 *
 * 3. FPS 제어 로직:
 *    - 디코딩 속도가 설정된 FPS보다 빠를 경우, `std::this_thread::sleep_for`를
 * 통해 대기 시간을 삽입하여 불필요한 자원 소모를 방지합니다.
 */
