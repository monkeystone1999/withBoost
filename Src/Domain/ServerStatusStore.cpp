...
double ServerStatusStore::deviceCpu(const std::string &ip) const {
    std::lock_guard<std::mutex> lk(m_mutex);
    for (auto &d : m_status.devices) if (d.ip == ip) return d.cpu;
    return 0.0;
}
double ServerStatusStore::deviceMemory(const std::string &ip) const {
    std::lock_guard<std::mutex> lk(m_mutex);
    for (auto &d : m_status.devices) if (d.ip == ip) return d.memory;
    return 0.0;
}
double ServerStatusStore::deviceTemp(const std::string &ip) const {
    std::lock_guard<std::mutex> lk(m_mutex);
    for (auto &d : m_status.devices) if (d.ip == ip) return d.temp;
    return 0.0;
}
int ServerStatusStore::deviceUptime(const std::string &ip) const {
    std::lock_guard<std::mutex> lk(m_mutex);
    for (auto &d : m_status.devices) if (d.ip == ip) return d.uptime;
    return 0;
}
