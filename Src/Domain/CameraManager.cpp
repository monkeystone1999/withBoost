...
void CameraImage::Feed(const ImageHeader &header, const uint8_t *imageData,
                       size_t imageSize) {
  auto &slot = pendingFrames_[header.FrameNumber];
  slot.maxSequence = header.MaxSequenceNumber;
  slot.fragments[header.SequenceNumber].assign(imageData,
                                               imageData + imageSize);
}
...
bool CameraImage::isFrameComplete(uint8_t frameNumber) const {
  ...
  if (it == pendingFrames_.end())
    return false;
  return static_cast<uint8_t>(it->second.fragments.size()) ==
         static_cast<uint8_t>(it->second.maxSequence + 1);
}
...
void CameraManager::Remove(uint32_t id) {
  ...
  if (ipTable_[info.ip_].empty())
    ipTable_.erase(info.ip_);
 ...
 *
 * **[CameraManager 내부 로직 상세]**
 *
 * 1. 가상 ID 및 주소 체계:
 *    - `ipTable_`: 각 IP별로 사용 중인 인덱스 세트를 관리하여 중복 없는 채널
 * 번호를 할당합니다.
 *    - `IpToId_`: "192.168.0.x/0" 형태의 문자열 키를 사용하여 고유한 정수 ID로
 * 매핑합니다.
 *
 * 2. 이미지 파이프라인:
 *    - `Feed`: 조각 데이터를 맵에 삽입.
 *    - `isFrameComplete`: (수신 조각 수 == MaxSequence + 1) 조건을 확인.
 *    - `ExtractFrame`: 맵의 조각들을 순서대로 병합하여 벡터 생성 후
 * `ImageList_` 보관.
 *
 * 3. NACK 기반 재전송:
 *    - `BuildResultJson`은 비어있는 시퀀스 번호를 찾아 JSON 배열(`loss`)로
 * 반환합니다.
 *    - 네트워크 레이어는 이 정보를 바탕으로 서버에 특정 조각의 재전송을 요청할
 * 수 있습니다.
 */
